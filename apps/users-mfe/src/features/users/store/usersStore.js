...
export const useUsersStore = create()(persist((set, get) => ({
    search: '',
    page: 1,
    pageSize: 12,
    localUsers: [],
    ...
    addUser: (user) => set((s) => ({ localUsers: [user, ...s.localUsers], page: 1 })),
    updateUser: (user) => {
        const isLocal = get().localUsers.some((u) => u.login.uuid === user.login.uuid);
        if (isLocal) {
            set((s) => ({
                localUsers: s.localUsers.map((u) => u.login.uuid === user.login.uuid ? user : u),
            }));
        }
        else {
